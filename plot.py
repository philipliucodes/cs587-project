...
load_path_1 = (
    load_path + "_" + str(1) + "_" + str(args.batch_size) + "_" + str(args.ssize)
)
...
for i in range(args.job_id_max):
    pl_result_0[:, :, :, i] = np.load(
        load_path_0 + "_" + str(i + 1) + "_" + "pl" + ".npy"
    )
    pl_result_1[:, :, :, i] = np.load(
        load_path_1 + "_" + str(i + 1) + "_" + "pl" + ".npy"
    )
...
test_err_0_mean, test_err_0_std = np.mean(test_err_0, axis=1), np.std(
    test_err_0, axis=1
)
test_err_1_mean, test_err_1_std = np.mean(test_err_1, axis=1), np.std(
    test_err_1, axis=1
)

train_loss_0_mean, train_loss_0_std = np.mean(train_loss_0, axis=1), np.std(
    train_loss_0, axis=1
)
train_loss_1_mean, train_loss_1_std = np.mean(train_loss_1, axis=1), np.std(
    train_loss_1, axis=1
)
...
locmin_1 = mpl.ticker.LogLocator(
    base=10.0, subs=np.arange(0, 1.0, step=ystep_1), numticks=12
)
...
axs[0].plot(
    epoch, test_err_0_mean[start_epoch:end_epoch], "c", label=label_0, linewidth=3
)
axs[0].plot(
    epoch, test_err_1_mean[start_epoch:end_epoch], "k--", label=label_1, linewidth=3
)
...
axs[1].plot(
    epoch, train_loss_0_mean[start_epoch:end_epoch], "c", label=label_0, linewidth=3
)
axs[1].plot(
    epoch, train_loss_1_mean[start_epoch:end_epoch], "k--", label=label_1, linewidth=3
)
...
fig.savefig(
    plot_path + "_ep" + "_plt.png",
    bbox_inches=mpl.transforms.Bbox([[-0.66, -0.3], [7.8, 9.1]]),
    pad_inches=0.1,
)
